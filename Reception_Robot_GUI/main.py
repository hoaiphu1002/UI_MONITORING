...
class MainWindow(QMainWindow):
    ...
    def add_path_planning_buttons(self, location_tab):
        ...
        for btn, name in zip(buttons, goals):
            if name: # Tránh trường hợp name bị rỗng
                btn.setText(name)
    ...
    def send_goal(self, place: str):
        ...
        # 1. Hiển thị trạng thái lên UI ngay lập tức
        self.ui.robot_status.setText(f"Moving to {place}")
        self.ui.robot_status_2.setText(f"Moving to {place}")

        # 2. Gửi tín hiệu MQTT
        goal_json = json.dumps(place)
        GoalPublisher().publish_goal(goal_json)

        # 3. Kích hoạt vẽ đường đi và gửi lệnh xoay hướng ở location.py
